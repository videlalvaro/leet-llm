...
public enum WeightContainerError: Error, Equatable, LocalizedError {
  ...
  public var errorDescription: String? {
    ...
    case .truncatedPreamble(let expected, let actual):
      "LeetWeight requires at least \(expected) preamble bytes; received \(actual)."
    case .invalidMagic:
      "LeetWeight magic bytes do not match LLMWGT01."
    case .unsupportedVersion(let version):
      "LeetWeight version \(version) is unsupported."
    ...
    case .invalidUTF8Header:
      "LeetWeight header is not valid UTF-8."
    case .invalidJSONHeader(let message):
      "LeetWeight header is not valid schema-compatible JSON: \(message)"
