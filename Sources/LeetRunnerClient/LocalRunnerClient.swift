...
public enum LocalRunnerClientError: Error, LocalizedError, Equatable {
    ...
    public var errorDescription: String? {
        switch self {
        case .executableMissing:
            "The leetllm-runner executable could not be found."
...
public struct LocalRunnerClient: Sendable {
    ...
    public static func locate(
        ...
        var candidates = [
            applicationBundleURL.appending(path: "Contents/Helpers/leetllm-runner")
        ]
        if let applicationExecutableURL {
            candidates.append(
                applicationExecutableURL.deletingLastPathComponent()
                    .appending(path: "leetllm-runner")
            )
        }
        if let configuredPath = environment["LEETLLM_RUNNER_PATH"], !configuredPath.isEmpty {
