...
final class RunnerPipelineTests: XCTestCase {
    private static let environmentSentinel = "LEETLLM_TEST_PARENT_CREDENTIAL"
    ...
    func testLocatePrefersBundledRunnerOverEnvironmentOverride() async throws {
        try await withTemporaryDirectory { destinationRoot in
            let bundleURL = destinationRoot.appending(path: "LeetLLM Studio.app")
            let bundledRunnerURL = bundleURL.appending(
                path: "Contents/Helpers/leetllm-runner"
            )
            ...
            let client = try LocalRunnerClient.locate(
                environment: ["LEETLLM_RUNNER_PATH": overrideRunnerURL.path],
                applicationExecutableURL: bundleURL.appending(
                    path: "Contents/MacOS/leetllm-studio"
                ),
    ...
    func testRealRunnerBuildsAndChecksIsolatedP001Starter() async throws {
        guard let runnerPath = ProcessInfo.processInfo.environment["LEETLLM_RUNNER_PATH"] else {
            throw XCTSkip("Set LEETLLM_RUNNER_PATH to run the process integration test.")
        }
        try await withTemporaryDirectory { destinationRoot in
        ...
        try await withTemporaryDirectory { destinationRoot in
            ...
            try environmentCheckingP001Source.write(
                to: workspace.fileURL(
                    for: "Sources/LeetLLMExercises/P001VectorDotExercise.swift"
                ),
                atomically: true,
                encoding: .utf8
            )
